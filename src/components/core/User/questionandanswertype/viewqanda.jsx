...
import { Edit, Trash2, Filter, ArrowLeft, Plus, List, Upload, FileEdit, X } from "lucide-react";
import { getAllQandARegions, getMyCreatedQandA, deleteQandA, updateQandA } from "../../../../services/operations/qandA";
...
// Modal Component for changing chapter name - REMOVED as per requirements

export default function ViewMyQandA() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Q&A data
        const qandaResponse = await getMyCreatedQandA();
  ...
  // Apply filters whenever formData changes
  useEffect(() => {
    filterQandas();
  }, [formData, qandas]);
  ...
  const handleView = (qanda) => {
    navigate(`/user-qanda-detail/${qanda._id}`);
  };

  const handleEdit = (qanda) => {
    if (qanda.status === "active") {
      return; // Do nothing if status is active
    }
    navigate(`/edit-qanda/${qanda._id}`);
  };

  // REMOVED: handleChangeChapter function
  ...
  const isActionDisabled = (status) => status === "active";
          ...
          <div className="p-6 sm:p-8 border-b border-indigo-100 bg-gradient-to-r from-blue-50 to-cyan-50">
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                {/* Chapter Field - MOVED HERE after Status */}
                <div className="space-y-2">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      <AnimatePresence>
                        {filteredQandas.map((qanda, index) => {
                          const isActive = qanda.status === "active";
                          return (
                            <motion.tr
                              key={qanda._id}
                                ...
                                <div className="flex items-center justify-center gap-1">
                                  ...
                                  <div className="relative">
                                    <motion.button
                                      ...
                                      disabled={isActive}
                                      className={`p-2 rounded-lg transition-all duration-200 relative group ${isActive 
                                        ? 'bg-gray-300 cursor-not-allowed' 
                                        : 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white'
                                      }`}
                                      title={isActive ? "You cannot edit active Q&A" : "Edit Q&A"}
                                    >
                                      <Edit className="w-4 h-4" />
                                    </motion.button>
                                    {isActive && (
                                      <div className="absolute z-10 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-800 text-white text-xs rounded-lg whitespace-nowrap">
                                        You cannot Edit
                                  ...
                                  <div className="relative">
                                    <motion.button
                                      ...
                                      title={isActive ? "You cannot delete active Q&A" : "Delete Q&A"}
                                    >
