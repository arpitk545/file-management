...
import { getAllQuizRegions, updateQuizRegion, deleteQuizRegion } from "../../../../services/operations/quizAPI"
import { toast } from "react-hot-toast"
import { useParams } from "react-router-dom"

const EditQuizRegion = () => {
...
//Update the Quiz region by the name 
 ...
 const handleUpdateRegion = async () => {
  ...
  try {
    ...
    const response = await updateQuizRegion(regionName, {
      updatedData: selectedRegion
    });

    if (response.message) {
      toast.success(response.message);
      const updatedResponse = await getAllQuizRegions();
  ...
  const handleDeleteRegion = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await deleteQuizRegion(selectedRegion.name)
      if (response.message) {
        toast.success(response.message)
        const updatedResponse = await getAllQuizRegions()
        setRegions(updatedResponse?.data || [])
        ...
        updatedExamTypes[selectedExamTypeIndex] = {
          ...
          specificClasses: updatedExamTypes[selectedExamTypeIndex].specificClasses.map((sc, i) => {
            if (i === selectedSpecificClassIndex) {
              return {
                ...sc,
                subjects: sc.subjects.map((subj, j) => {
                  if (j === selectedSubjectIndex) {
                    return {
                      ...subj,
                      chapters: subj.chapters.map((chap, k) => (k === index ? { ...chap, name: newName } : chap)),
                    }
                  }
                  return subj
                }),
              ...
              return {
                ...
                subjects: sc.subjects.map((subj, j) => {
                  if (j === selectedSubjectIndex) {
                    return {
                      ...subj,
                      chapters: subj.chapters.filter((_, k) => k !== index),
                    }
                  }
                  return subj
                }),
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-4 sm:mb-6 md:mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">
            Modify existing quiz regions and their categories
          </p>
        </div>
            ...
            <div className="space-y-4 sm:space-y-6">
                      ...
                      <div className="flex items-center gap-1 sm:gap-2">
                        ...
                        {selectedRegionIndex === index ? (
                          <input
                            ...
                            className="flex-1 text-sm sm:text-base font-medium text-gray-800 dark:text-white bg-transparent border-b border-transparent focus:border-teal-300 focus:outline-none px-1"
              ...
              {selectedRegion && (
                <div className="space-y-4 sm:space-y-6 mt-4 sm:mt-6">
                  ...
                  {/* Specific Classes - Only show if an exam type is selected */}
                  ...
                  {/* Subjects - Only show if a specific class is selected */}
                  ...
                  {/* Chapters - Only show if a subject is selected */}
                  {selectedSubjectIndex !== null && (
                    <div className="ml-8 sm:ml-10 md:ml-12 border-l-2 border-purple-300 pl-2 sm:pl-4 md:pl-6 space-y-2 sm:space-y-4">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-2 md:gap-3 mb-2 sm:mb-4">
                        ...
                        <button
                          onClick={() => toggleNewCategoryInput("chapter")}
                      ...
                      {newCategoryInputs.chapter && (
                        <div className="flex flex-col sm:flex-row gap-3">
                          <input
                            type="text"
                            value={newCategoryValues.chapter}
                            onChange={(e) => handleNewCategoryChange("chapter", e)}
                            placeholder="Enter new chapter name"
                            className="flex-1 px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 transition-all duration-200"
                            onKeyPress={(e) => e.key === "Enter" && addNewCategory("chapter")}
                          />
                          <button
                            onClick={() => addNewCategory("chapter")}
                            className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-lg hover:from-blue-600 hover:to-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2"
                          >
                            <Plus className="w-4 h-4" />
                            Add
