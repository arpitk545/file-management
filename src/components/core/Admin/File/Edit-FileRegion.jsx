...
const EditFileRegion = () => {
  ...
  const handleUpdateRegion = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await UpdateFileRegion(selectedRegion.name, selectedRegion);
      
      if (response.message) {
        toast.success(response.message);
        const updatedResponse = await GetFileRegion();
        setRegions(updatedResponse?.data || []);
        const newIndex = updatedResponse?.data?.findIndex(r => r.name === selectedRegion.name);
  ...
  const updateRegionName = (e) => {
    ...
    setRegions(prev => prev.map((region, i) => 
      i === selectedRegionIndex ? { ...region, name: newName } : region
    ));
  };
